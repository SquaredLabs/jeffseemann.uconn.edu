...
class YearsNavigation extends Component {
  render () {
    ...
    */
    const { yearClickAction, yearsNav, showUntil } = this.props

    // Display years in navigation not already on screen
    const yearsNavFiltered = yearsNav.map((year, i) => {
      return <div className="navigation-year" key={i} onClick={() => showUntil(year)}>{year}</div>
    })
